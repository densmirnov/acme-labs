...
Входы зафиксированы для **18–22 мая 2026 года**. Для самостоятельного запуска начните с [пакета контекста](context-capsule.md); для подготовки и сравнения контекста — с [воркшопа](../../workshops/context-layer/README.md). Сохранённые версии ниже относятся к прежнему учебному примеру, а не к вашим новым попыткам.

Новый ответ сохраняйте в `runs/<run-id>/` с уникальным именем и запишите фактическое наблюдение в журнал. `runs/v0.1/` и `runs/v0.2/` в исторических контрактах обозначают существующие образцы; повтор по тому же контракту не должен их заменять. Версии контракта, пакета контекста и идентификатор запуска учитываются отдельно.
...
weekly-launch-update/
  ...
  context-capsule.md            выбранные входы для нового запуска
  run-log.md
  inputs/
